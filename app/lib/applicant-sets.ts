...
export type ApplicantSet = {
  ...
  /** How many graders this set gives each applicant. Fixed when the set is
   *  created, so a set already being graded keeps the arity it started under. */
  gradersPerApplicant: number;
  applicantCount?: number;
  activeGraderCount?: number;
};

/** The columns as Postgres returns them, before camel-casing. */
type SetRow = Omit<ApplicantSet, "gradersPerApplicant" | "applicantCount" | "activeGraderCount"> & {
  graders_per_applicant: number;
};

function toApplicantSet({ graders_per_applicant, ...set }: SetRow): ApplicantSet {
  return { ...set, gradersPerApplicant: graders_per_applicant };
}

export async function getActiveApplicantSet(): Promise<ApplicantSet | null> {
  const { data, error } = await supabase
    .from("applicant_sets")
    .select("id, name, version, status, graders_per_applicant")
    .eq("status", "active")
    .maybeSingle<SetRow>();
  if (error) throw new Error(`Could not load active applicant set: ${error.message}`);
  return data ? toApplicantSet(data) : null;
}
...
export async function listApplicantSets(): Promise<ApplicantSet[]> {
  ...
  const sets = ((data ?? []) as SetRow[]).map(toApplicantSet);
